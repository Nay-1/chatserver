...
bool Redis::subscribe(int channel)
{
    ...
    // redisAppendCommand只是把命令发送到缓冲区，并没有真正发送到redis server
    ...
    while (!done)
    {
        // redisBufferWrite会把redisAppendCommand命令发送到redis server，直到发送完毕返回REDIS_OK，否则返回REDIS_ERR
...
void Redis::observer_channel_message()
{
    ...
    // redisGetReply会阻塞等待redis server响应消息，直到收到订阅通道的消息才会返回
